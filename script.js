    ...
    <head>
      ...
      <style>
        ...
        .mensagem-descritor {
          ...
          background: linear-gradient(135deg, #f5e6c8, #f6c0a8, #fa8072);
          color: #000000;
          font-weight: bold;
        }
...
function gerarBancoPadrao() {
  ...
  const bancoFiltrado = bancoQuestoesAulas17a25.filter(questao => {
    return aulasPermitidas.has(obterNumeroAula(questao.aula));
  });

  return embaralhar(bancoFiltrado.map(prepararQuestao)).slice(0, TOTAL_QUESTOES_BANCO);
}
...
function gerarDistratoresObjetivos(fraseCorreta, tema, frases) {
  ...
  for (const frase of frases) {
    ...
    if (!distratores.some(item => normalizarTermo(item) === normalizarTermo(alternativa))) {
...
function extrairTermosImportantes(texto) {
  ...
  palavras.forEach(palavra => {
    ...
    if (!unicas.some(item => normalizarTermo(item) === normalizada)) {
      unicas.push(palavra);
    }
  });
...
function removerAlternativasRepetidas(alternativas) {
  const vistas = new Set();
  const resultado = [];
...
function palavrasIgnoradas() {
  return [
    ...
    "esse", "essa", "este", "aquele", "aquela", "deve", "podem", "pode",
    "cada", "qual", "quais", "toda", "todo", "todos", "todas", "mesmo",
    "dele", "dela", "nossa", "nosso", "outros", "outra", "outras", "outro",
    "sendo", "tendo", "forma", "maneira", "segundo", "assim", "portanto",
    "conteúdo", "arquivo", "texto", "material", "questão", "questões",
